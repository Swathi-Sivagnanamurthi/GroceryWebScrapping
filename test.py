...
if search:
    ...
    data_price=pd.concat([walmart, food_basics], ignore_index=True)
    data_price['Price'] = data_price['Price'].apply(lambda x: f'${x}')
...
if 'compare_clicked' in st.session_state and st.session_state['compare_clicked']:
    ...
    if df is not None:
        ...
        fig = px.bar(chart_data,x='Brand',color='store',
